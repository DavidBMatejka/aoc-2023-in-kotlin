...
fun main() {
    ...
    data class Node(val x: Int, val y: Int) {
        ...
        var steps: Int = 0
        fun possibleNeighbours(grid: Array<Array<Char>>): List<Node> {
            ...
            if (x + 1 < grid[0].size && (grid[y][x + 1] == '.' || grid[y][x + 1] == '>')) {
                val tmp = Node(x + 1, y)
                if (tmp != this.prev) {
                    tmp.prev = this
                    neighbours.add(tmp)
            ...
            if (y + 1 < grid.size && (grid[y + 1][x] == '.' || grid[y + 1][x] == 'v')) {
                val tmp = Node(x, y + 1)
                if (tmp != this.prev) {
                    tmp.prev = this
                    neighbours.add(tmp)
    ...
    fun toGraph(input: List<String>): Pair<MutableList<Vertex>, MutableMap<Vertex, MutableSet<Edge>>> {
        ...
        while (q.isNotEmpty()) {
            ...
            val neighbours = current.possibleNeighbours(grid)
    ...
    fun part1(input: List<String>): Int {
        ...
        val startVertex = Vertex(1, 0)
        val (vertices, edges) = toGraph(input)
